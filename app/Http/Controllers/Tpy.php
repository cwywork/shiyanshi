...
class Tpy extends Controller
{
    public function toTpy(Request $request)
    {
        $data = DB::select('select id, sch_name from schools');
        $tmp = '';
        foreach ($data as $k => $v) {
            $fall = $this->encode($v->sch_name, 'head');
            $all  = $this->encode($v->sch_name, 'all');
            $tmp .= " update schools set full_letters = '{$all}', first_letters = '{$fall}' where id={$v->id};";
        }
        echo $tmp;
    }
